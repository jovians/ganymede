...
const getUrlInfo = (url: string): UrlInfo => {
  ...
  if (!env.hostname) { env.hostname = location.hostname; }
...
self['newClientChecker'] = () => {
  ...
  self.clients.matchAll({includeUncontrolled: true, type: 'window'}).then(matchedClients => {
    if (matchedClients && matchedClients.length) {
      ...
      const announce = (client, cid) => {
        client.postMessage({
          type: 'NEW_CLIENT', cid, message: 'GanymedeSW::ACK ' + cid
        });
      };
...
fetchFunc = e => {
  try {
    ...
    if (info.path === '/') {
      return e.respondWith(caches.match('/').then(r => {
        if (r) { return r; }
        return fetch(e.request.url, fetchParam).then(response => {
