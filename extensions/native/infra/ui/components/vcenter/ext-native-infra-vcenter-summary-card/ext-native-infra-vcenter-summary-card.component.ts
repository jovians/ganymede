...
@Component({
  selector: 'gany-ext-native-infra-vcenter-summary-card',
  templateUrl: './ext-native-infra-vcenter-summary-card.component.html',
  styleUrls: ['./ext-native-infra-vcenter-summary-card.component.scss'],
})
export class ExtNativeInfraVcenterSummaryCardComponent extends ix.Entity implements OnInit, OnDestroy, OnChanges {
  ...
  hydrate(nocache = false) {
    ...
    switch (dat.type) {
      ...
      case 'vsphere':
      case 'vcenter': {
        rx.invoke(store.extInfra.vcenter.quickStats.actions.FETCH, { key: dat.key, nocache });
